...
#token ID "M[1-9]* | [a-zA-LN-Z][a-zA-Z0-9]*" //Considero que en el cas d'una muntanya ha de començar per M i el cas d'una variable no
...
plusMinusOp: productDivOp ((PLUS^ | HIPHEN^) productDivOp)*;
productDivOp: operand ((STAR^ | BAR^) operand)*;
operand: symbol | ID | LPAR! plusMinusOp RPAR!;
symbol: NUM ( | AND^ (BAR | HIPHEN | DOWN));

peakOrValley: CREATEFUNC^ LPAR! assignedValue COMMA! assignedValue COMMA! assignedValue RPAR!;
