...
#endif
{
  ...
  {
  {
    ...
    for (;;) {
      ...
      if ( (LA(1)==ID) ) {
        assignExecute(zzSTR); zzlink(_root, &_sibling, &_tail);
      }
      else {
        if ( (LA(1)==MOVE) ) {
...
#endif
{
  ...
  zzmatch(LPAR);  zzCONSUME;
  condicBool(zzSTR); zzlink(_root, &_sibling, &_tail);
  zzmatch(RPAR);  zzCONSUME;
...
#endif
{
  ...
  zzMake0;
  {
  condicBool2(zzSTR); zzlink(_root, &_sibling, &_tail);
  {
    zzBLOCK(zztasp2);
    zzMake0;
    {
